...
def _merge_ms_s45c(categories: list) -> list:
    """Combine MS (bandsaw) and S45C into a single sheet (Monica version)."""
    ms_idx = s45c_idx = None
    for i, cat in enumerate(categories):
        kws = [k.upper() for k in cat["keywords"]]
        if "MS" in kws and ms_idx is None:
            ms_idx = i
        if "S45C" in kws and s45c_idx is None:
            s45c_idx = i
    if ms_idx is not None and s45c_idx is not None and ms_idx != s45c_idx:
        ms_cat   = categories[ms_idx]
        s45c_cat = categories[s45c_idx]
        combined_kws = list(ms_cat["keywords"]) + [
            kw for kw in s45c_cat["keywords"] if kw not in ms_cat["keywords"]
        ]
        combined = {
            "name":     "MS (bandsaw) / S45C",
            "sheet":    "MS_S45C",
        ...
        for idx in sorted([ms_idx, s45c_idx], reverse=True):
            categories.pop(idx)
        categories.insert(min(ms_idx, s45c_idx), combined)
    return categories


def get_categories() -> list:
    """Return categories from the bundled xlsx if available, otherwise use built-ins.
    MS (bandsaw) and S45C are always combined into one sheet."""
    if MAT_SUP_PATH.exists():
        try:
            cats = _parse_categories_from_xlsx(str(MAT_SUP_PATH))
            return _merge_ms_s45c(cats)
        except Exception:
            pass
